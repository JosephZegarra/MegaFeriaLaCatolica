...
public class PagosServiceImpl implements PagosService {
    ...
    public PagosDTO registrarpago(Integer iddeuda, BigDecimal monto, String numero_carnet) {
        ...
        DeudaEntity deudaEntity = deudaRepository.findById(iddeuda)
                .orElseThrow(() -> new RuntimeException("La deuda con id" + " " + iddeuda + "no existe"));
        ...
        if (monto.compareTo(saldoPendiente) > 0) {
            throw new RuntimeException(
                    "El monto ingresado (" + monto + ") excede el saldo pendiente (" + saldoPendiente + ").");
        }
    ...
    public List<PagosDTO> obtenerPagosPorSocio(Integer idsocio) {
        ...
        List<PagosDTO> pagosDTO = pagos.stream().map(pago -> {
            ...
            if (pago.getCarnetRegistrador() != null) {
                SocioEntity socioEntity = socioRepository.findByNumeroCarnet(pago.getCarnetRegistrador())
                        .orElseThrow(() -> new RuntimeException(
                                "No se encontró el usuario con carnet " + pago.getCarnetRegistrador()));
                dto.setNombreRegistrador(socioEntity.getNombresocio());// Lo que hace stream es separar en identidades
                                                                       // y map para que cada pago dto funcione de forma
                                                                       // separada
    ...
    public List<PagosDTO> obtenerTodosLosPagos() {
        List<PagosEntity> pagos = pagosRepository.findAllPagosOrderByFechaDesc();
        List<PagosDTO> pagosDTO = pagos.stream().map(pago -> {
            PagosDTO dto = pagosMapper.PagosEntityAPagosDTO(pago);

            // Agregar nombre del registrador si existe
            if (pago.getCarnetRegistrador() != null) {
                SocioEntity socioRegistrador = socioRepository.findByNumeroCarnet(pago.getCarnetRegistrador())
                        .orElseThrow(() -> new RuntimeException(
                                "No se encontró el usuario con carnet " + pago.getCarnetRegistrador()));
                dto.setNombreRegistrador(socioRegistrador.getNombresocio());
            }

            // Agregar información del socio que tiene la deuda
            if (pago.getDeuda() != null && pago.getDeuda().getSocioAsistenciaEntity() != null) {
                SocioEntity socioDebe = pago.getDeuda().getSocioAsistenciaEntity().getSocio();
                dto.setNombreSocio(socioDebe.getNombresocio());
                dto.setApellidoSocio(socioDebe.getApellidopaternosocio());
            }
