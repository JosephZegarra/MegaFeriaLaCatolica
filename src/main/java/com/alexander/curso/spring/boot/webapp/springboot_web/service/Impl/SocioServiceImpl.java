...
public class SocioServiceImpl implements SocioService { // PARA MOSTRAR al usuario lo convierto a DTO
    ...
    public List<SocioDTO> listar() {
        ...
        // return
        // SocioMapper.INSTANCE.socioDTOASocioDTO(socioRepository.findAll());//convierte
        // de lista de socios entity ha lista de socios DTO
        return socioRepository.findByEstado("Activo")
                .stream()
                .map(s -> new SocioDTO(s.getIdsocio(), s.getNombresocio(), s.getApellidopaternosocio(),
                        s.getDireccion(), s.getTelefono()))
                .toList();
    }

    @Override
    public SocioDTO BuscarPorId(Integer idsocio) {
        Optional<SocioEntity> socioEntity = Optional.ofNullable(socioRepository.findById(idsocio)
    ...
    public SocioDTO guardar(SocioDTO socioDTO) {
        ...
        if (socioRepository.existsByNumero_carnet(socioDTO.getNumero_carnet())) {
            // throw new IllegalArgumentException("El numero de carnet ya está registrado.
            // Ingrese otro numero de carnet");
        ...
        // Código que identifica si el correo puso el cliente para generar token
        // automatico
        ...
        if (savedEntity.getCorreo() != null && !savedEntity.getCorreo().isEmpty())// isEmpty verifica que la cadena este
                                                                                  // vacia
        {
            try {
                ...
                // Error al enviar correo - no loguear información sensible del usuario
            }
        }

        // Enviar correo
        // correoService.enviarCorreo(savedEntity.getCorreo(),"Registro
        // exitoso","Hola"+" "+socioEntity.getNombresocio()+", tu registro en el sistema
        // de la Mega Feria de la Católica fue éxitoso ");
    ...
    public SocioDTO editar(SocioDTO socioDTO) {
        ...
        SocioEntity socioEntity = socioRepository.findById(socioDTO.getIdsocio())
                .orElseThrow(() -> new EntityNotFoundException("Socio no encontrado"));
    ...
    public SocioDTO validarSocio(String numero_carnet, String contrasena) {
        ...
        // Validar contraseña sin loguear información sensible
    ...
    public SocioDTO BuscarPorNumero_Carnet(String numero_carnet) {
        /*
         * SocioEntity
         * socioEntity=socioRepository.findByNumeroCarnet(numero_carnet).orElseThrow(()
         * -> new RuntimeException("No se encontró un socio con el número de carnet: " +
         * numero_carnet));
