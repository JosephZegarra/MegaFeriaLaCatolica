...
public interface MovimientoRepository extends JpaRepository<MovimientoEntity, Long> {
       ...
       * @Query("""
       * SELECT new
       * com.alexander.curso.spring.boot.webapp.springboot_web.dto.MovimientoDTO(
       * m.fecha,
      ...
      // Obtener movimientos por mes y año específicos
      @Query("SELECT m FROM MovimientoEntity m " +
                  "WHERE EXTRACT(YEAR FROM m.fecha) = :anio " +
                  "AND EXTRACT(MONTH FROM m.fecha) = :mes " +
                  "ORDER BY m.fecha")
      List<MovimientoEntity> findByMesYAnio(
                  @Param("mes") int mes,
                  @Param("anio") int anio);
}
