...
public interface SocioAsistenciaRepository extends JpaRepository<SocioAsistenciaEntity, Integer> {
        ...
        /*
         * @Query("""
         * SELECT new com.alexander.curso.spring.boot.webapp.springboot_web.dto.
         * ResumenAsistenciaDTO(
         * CAST(a.fecha AS date),
         * COUNT(sa.socio.idsocio),
         * SUM(CASE WHEN sa.estado = 'Ausente' THEN 1 ELSE 0 END),
         * SUM(CASE WHEN sa.estado = 'Ausente' AND sa.motivo IS NOT NULL THEN 1 ELSE 0
         * END)
         * )
         * FROM SocioAsistenciaEntity sa
         ...
         * ResumenAsistenciaDTO(
         * CAST(a.fecha AS date),
         * COUNT(DISTINCT CASE WHEN sa.estado = 'Presente' THEN sa.idSocioAsistencia
         * END),
         * COUNT(DISTINCT CASE WHEN sa.estado = 'Ausente' THEN sa.idSocioAsistencia
         * END),
         * COUNT(DISTINCT CASE WHEN sa.estado = 'Ausente' AND sa.motivo <> '' THEN
         * sa.idSocioAsistencia END)
         * )
         * FROM SocioAsistenciaEntity sa
         * JOIN sa.asistencia a
         * WHERE CAST(a.fecha AS date) BETWEEN :from AND :to
        ...
        // Obtener asistencias por mes y año específicos
        @Query("SELECT sa FROM SocioAsistenciaEntity sa " +
                        "JOIN sa.asistencia a " +
                        "WHERE EXTRACT(YEAR FROM a.fecha) = :anio " +
                        "AND EXTRACT(MONTH FROM a.fecha) = :mes " +
                        "ORDER BY a.fecha")
        List<SocioAsistenciaEntity> findByMesYAnio(
                        @Param("mes") int mes,
                        @Param("anio") int anio);
